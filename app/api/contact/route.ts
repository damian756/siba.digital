...
function buildHtml(category: Category, data: Record<string, string>): string {
  ...
  const rows = Object.entries(data)
    ...
    .map(
      ([k, v]) =>
        `<tr><td style="padding:6px 12px 6px 0;font-weight:600;color:#4a4a4a;white-space:nowrap;">${k}</td><td style="padding:6px 0 6px 12px;color:#1c1c1c;">${v.replace(/\n/g, "<br>")}</td></tr>`
    )
    .join("");

  return `
    <div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#1c1c1c;padding:24px 28px;border-radius:8px 8px 0 0;">
